...
def save_checkpoint(session, model, iteration, config):
    print()
    print()
    print('#############')
    print('# checkpoint!')
...
def load_checkpoint(session, model, config):
...
def test_samples(session, model, name, config):
    zs, cs = session.run([
    ...
    x_generated = session.run(
        model.g, feed_dict={model.z_in: zs, model.c_in: cs}
    )
